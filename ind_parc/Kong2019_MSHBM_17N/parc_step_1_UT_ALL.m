...
%Read in the subjids text file
    filename = '/nobackup/scratch/grp/fslg_spec_networks/code/Utah_analysis/Kong2019_parc_fs6_ALL/subjids/subjids.txt';
    delimiter = {''};
...
project_dir = '/fslgroup/grp_proc/compute/Utah_analysis/parc_output_fs6_UT_ALL/generate_profiles_and_ini_params'
for subid = 1:length(sublist)
     sub=sublist(subid);
     ses1=strcat('lh.sub-', sub, '_bld001_rest_skip4_mc_me_resid_fs6_sm6_fs6.nii.gz');
   	 str1 = fullfile(preproc_dir,strcat('sub-', sub), strcat('sub-', sub), 'surf', ses1);
     ses2=strcat('lh.sub-', sub, '_bld002_rest_skip4_mc_me_resid_fs6_sm6_fs6.nii.gz');
     str2 = fullfile(preproc_dir,strcat('sub-', sub), strcat('sub-', sub), 'surf', ses2);
     if isfile(str1) && isfile(str2)
            for sess = seslist
...
%% Part 2: Create group profile- ASD
%Edit CBIG_MSHBM_avg_profiles to include SUBJID list and any pertinent
%prefixes (ex: "MSC")

project_dir = '/fslgroup/grp_proc/compute/Utah_analysis/parc_output_fs6_UT_ALL/generate_profiles_and_ini_params'
num_sub = '60';  %point to correct subjids.txt file
num_sess = '2'; %max number of sessions
...
%% Part 2: Create group profile- TD
%Edit CBIG_MSHBM_avg_profiles to include SUBJID list and any pertinent
%prefixes (ex: "MSC")

project_dir = '/fslgroup/grp_proc/compute/Utah_analysis/parc_output_fs6_UT_ALL/generate_profiles_and_ini_params'
num_sub = '85';  %point to correct subjids.txt file
num_sess = '2'; %max number of sessions
CBIG_MSHBM_avg_profiles('fsaverage3','fsaverage6',project_dir,num_sub,num_sess);
