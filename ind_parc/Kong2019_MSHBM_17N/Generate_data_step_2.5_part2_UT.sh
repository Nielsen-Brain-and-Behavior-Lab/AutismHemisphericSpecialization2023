...
HOME_D=/fslgroup/grp_proc/compute
out_dir=${HOME_D}/Utah_analysis/parc_output_fs6_UT_ALL
...
lh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile.nii.gz
		FILE2=${out_dir}/generate_profiles_and_ini_params/profiles/sub${sub}/sess${sess}/\
lh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile_1.nii.gz
		FILE3=${out_dir}/generate_profiles_and_ini_params/profiles/sub${sub}/sess${sess}/\
lh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile_2.nii.gz
...
rh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile.nii.gz"
		...
		lh_profile=$sub_split #doesn't matter what is echoed; just counting lines
		echo $lh_profile >> $sess_dir/sub-${sub}_lh.txt
		echo $lh_profile >> $sess_dir/sub-${sub}_lh.txt #x2- one for each split
	
		rh_profile=$sub_split #doesn't matter what is echoed; just counting lines
		echo $rh_profile >> $sess_dir/sub-${sub}_rh.txt
		echo $rh_profile >> $sess_dir/sub-${sub}_rh.txt #x2- one for each split
