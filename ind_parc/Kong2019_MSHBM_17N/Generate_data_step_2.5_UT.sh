...
for sub in `cat ${code_dir}/subjids/subjids.txt`; do
	sess1=`ls -A $out_dir/generate_profiles_and_ini_params/profiles/sub${sub}/sess1`
	sess2=`ls -A $out_dir/generate_profiles_and_ini_params/profiles/sub${sub}/sess2`
	test1=0
	test2=0
	[ -z "$sess1" ] && test1=1 #1 if empty
	[ -z "$sess2" ] && test2=1 #1 if empty
...
rh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile.nii.gz"
		...
		for sess in 1 2; do
			for split in 1 2; do
			FILE=$out_dir/generate_profiles_and_ini_params/profiles/sub${sub}/sess${sess}/\
lh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile_${split}.nii.gz
...
lh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile_${split}.nii.gz"
				...
				rh_profile="$out_dir/generate_profiles_and_ini_params/profiles/sub${sub}/sess${sess}/\
rh.sub${sub}_sess${sess}_fsaverage6_roifsaverage3.surf2surf_profile_${split}.nii.gz"
				echo $rh_profile >> $sess_dir/sub-${sub}_rh.txt
